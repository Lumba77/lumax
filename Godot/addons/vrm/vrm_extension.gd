...
func _update_materials(vrm_extension: Dictionary, gstate: GLTFState) -> void:
	...
	for i in range(materials.size()):
		...
		if oldmat is ShaderMaterial:
			...
			continue
		var newmat: Material = _process_khr_material(oldmat, gstate.get_json().get("materials", [])[i])
...
func _import_preflight(gstate: GLTFState, extensions: PackedStringArray = PackedStringArray(), psa2: Variant = null) -> Error:
	...
	if vrm_utils.is_processed(gstate.get_base_path()):
		return ERR_SKIP
	vrm_utils.mark_processed(gstate.get_base_path())
	var gltf_json_parsed: Dictionary = gstate.get_json()
	var gltf_nodes = gltf_json_parsed.get("nodes", [])
...
func _import_post(gstate: GLTFState, node: Node) -> Error:
	...
	var gltf_json: Dictionary = gstate.get_json()
	var extensions = gltf_json.get("extensions", {})
	var vrm_extension: Dictionary = extensions.get("VRM", {})
	...
	var humanoid = vrm_extension.get("humanoid", {})
	for human_bone in humanoid.get("humanBones", []):
