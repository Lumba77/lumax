...
static func get_palm_offset(mode : HandOffsetMode, xr_controller : XRController3D) -> Transform3D:
	...
	if xr_controller:
		...
		if mode != HandOffsetMode.HAND_OFFSET_AUTO:
			pass
		elif xr_controller.pose == "aim":
			mode = HandOffsetMode.HAND_OFFSET_AIM
		elif xr_controller.pose == "grip":
			mode = HandOffsetMode.HAND_OFFSET_GRIP
		else:
			# Assume we're using a palm pose
			mode = HandOffsetMode.HAND_OFFSET_PALM

	match mode:
		HandOffsetMode.HAND_OFFSET_AUTO:
			# No controller? keep identity transform
			pass
		HandOffsetMode.HAND_OFFSET_AIM:
			...
			if is_left_hand:
				...
				transform.origin = Vector3(0.02, -0.05, 0.10)
		HandOffsetMode.HAND_OFFSET_GRIP:
		...
		HandOffsetMode.HAND_OFFSET_PALM:
